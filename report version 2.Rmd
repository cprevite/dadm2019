...
#change class to date for data variable 
...
# we have to decide if we want to reèlace only factor or all variables with this method...
library(DMwR)
...
# # create de mode function
# Mode <- function(x) {
#   ux <- unique(x)
#   ux[which.max(tabulate(match(x, ux)))]
# }
# 
# #replace missing values: mean for numerical variables and mode for the categorical ones
# bands<- as.data.frame( lapply(bands, function(x) {
...
variables_to_keep <- table_result %>%
  filter(table_result$Decision == 'Confirmed' | table_result$Decision == 'Tentative')
...
lmProfile <- rfe(x=trainData, y=trainData$band_type,
                 sizes = subsets,
                 rfeControl = ctrl)
...
confusionMatrix(reference = testData$band_type, data = predicted, mode='everything', positive='NOBAND')
```
...
model_lr <- train(band_type ~ ., data = trainData, method="glm",trControl=fitControl)
...
mat <- confusionMatrix(data=pred,reference = testData$band_type, positive = "NOBAND")
print(mat)
```
...
model_svm <- train(band_type ~ ., data =
trainData, method = "svmLinear", trControl = fitControl, tuneGrid = data.frame(C=0.1))
print(model_svm)

print(confusionMatrix(data = predict(model_svm, newdata =
testData),reference = testData$band_type, positive = "NOBAND"))
```
