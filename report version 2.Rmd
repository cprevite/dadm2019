...
```
§
The results of the Boruta algorithm are shown in Figure 5 and 6, with the importance of each variables and the final decision of either to keep the variable or not. The first thing that can be observed is that there are a lot of important variables but a certain number of them show a higher importance than others, especially the right hand side of the elbow. Moreover, within the top 12 most important variables, only one (**grain_screened**) is impacted by the problem of the last 54 observations presenting only missing values. Running the Boruta algorithm on the database with the 485 observations makes sense, as the objective is to spot which variables are the most important and it is better to do it on a database which reflects the most the reality, limiting the missing values. 
...
In order to prevent a bad value in specificity in the further models, it has been decided that the number of each response variable need to be the same. This way, more weight is given to negative class **BAND**.

```{r}

table(bands2$band_type)

# Dataset of only 1s
bands.bands = bands2[bands2$band_type == "BAND",]

# Dataset with only 0s
bands.nobands = bands2[bands2$band_type == "NOBAND",]
...
plot_boxplot(bands.sample, by= 'band_type',  ncol = 2, title = "Side-by-side boxplots")
...
```{r}
integ_mod_1 <- data_integrity_model(data = bands.sample, model_name = "randomForest")
integ_mod_1$data_ok
...
fitControl <- trainControl(method = "cv", sampling = 'down')
