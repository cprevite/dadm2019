...
The variable **ink_type** could be also determinant in the processus as when the value is **UNCOATED** there is a high probability that the product would comes out as defectuous.
...
The third model that has been considered is the Logistic regression. Logistic regression is binomial regression model widely used in machine learning when the dependent variable is categorical. As in the case of the other models, the model has been buid using caret.
...
```
The secont SVM model offers good percentages in terms of Accuracy, Sensitivity and Specificity. 
...
confusion_matrix_NN
```
The Neural Network has less good results compared to teh other mdoels. This is not surprising, as normally this type of models works better when the dataset las big amount of datas to learn on, and in our case the data set has a total of 363 instances (75% of the total of 485 data points).
...
```
This model also provides a visualisation of the features that are considered the most importantfeatures, but as the model fails to provide good results in terms of predcitions, it is considered that the information would not be very helpful.
...
- on the right hand side there are some really good predictions with a lot of observations in this categorie (see percentage)
- also success to predict a defectuous product with 100% of propbability (the 4th categorie) 
...
Method <- c("Random Forest", "Nearest Neighbour", "Logistic regression", "Support Vector Machine 1", "Support Vector Machine 1", "Neuronal Network", "Classification Tree")
...
As for now the random forest has offred the best results in terms of accuracy, sensitivity and specificity. As the model uses over 20 variables, it is considered that it has a hight level of complexity. For this reason, a new model is proposed, one that uses the first 10 variables that resulted to be the most important at predicting the variable of interest suing the Random Forest algortithm. This information can be retrieved from the plot below.
...
Next, a new model has been build using the newly created data set. The model uses the Random Forest algorithm. The steps related to the data partion into a train and test set have been applied here as well. For the train control, the previous option have been 
