...
Here below is an overview of the dataset. The first table shows a summary of the data base, and the second table allows the user to explore the whole table. 
...
```{r datatable, echo=FALSE, message=FALSE, warning=FALSE,  fig.align='center'}
kable(bands, 'html') %>%
...
Figure 3 is confirming the previously stated intuition. The reparation of missing values seems to be concentrated in the very last observations for several variables. This indicates that chunks of consecutive values are missing. This could be due to a problem in recording the observations, or more simply due to the fact that from a certain point in time, the company decided  not to record these specific variables anymore. (*Note. In a real case scenario, the analysts would have the possibility to ask more information about this subject).
...
A first look at the database after all the modifications that have been done until now.
...
After a close verification of the box-plots and the summary for each variable, it has been concluded that there are no error measurements and neither error entries in the data. Please refer to the Annex1, to see the boxplots.
...
The Variables have been plotted against the variables of interest, 'band_type'.
...
Some interesting results comes from these graphs. For example, in the graph for **press**, there is a variation depending on the class of the observations: high and low values for **press** shows a prevalence of NOBAND and for intermediate value the prevalence is on the class BAND. This variable could be a serious candidate to predict the new observations.

The variable **cylinder_no** is also one important variable. These graph show that some of the cylinders have serious problem, specially cylinder from the family A: all the observations from this cylinder are defectuous. It is also the case for family V, but in a smaller measure, in the sense that these cylinder are pretty rare. Cylinder X is the most susceptible to be defectuous but it is also the most common in the dataset.

The variable **ink_type** could be also determinant in the process as when the value is **UNCOATED** there is a high probability that the product would comes out as defectuous.
...
All the models have been created using the 'caret' package. The **trControl** option of the **caret** package,  allows to include  computational nuances of the train function. For this  model a **repeatedcv** option has been chosen as the method of resampling.  This implies that the training dataset will be divided randomly into 10 parts. Each chunk will serve in turns as a test set for the model that has been build using the other nine datasets (which served as a train set). This **repeatedcv** option leads to robust models and for this reason the method will be also present for the other models. 
...
The Random Forest model gives the possibility to view the variables of the dataset that are perceived as important for the selected model. This information could be used to build new models that are using only the variables that have been labeled as important, thus reducing the complexity of the model.
...
```{r}
confusion_matrix_RF <- confusionMatrix(prediction_RF, testData$band_type, positive = 'NOBAND')
confusion_matrix_RF
```

The first model gives an accuracy of 77%, with sensitivity reaching 76% and specificity being 79%. This first results are encouraging, as the class that interests most is "BAND" and the model is able to predict 34 times correct out of a total of 43 instances. 
...
plot_var_RF =plot(varimp_rf, main = "Variable Importance with Random Forest")
plot_var_RF
...
The next model used in the analysis in the Nearest Neighbor Classifier, 
...
```{r}
confusion_matrix_KNN <- confusionMatrix(test_pred_knn, testData$band_type, positive = 'NOBAND')
confusion_matrix_KNN
```

The second  model provided an accuracy of 69%, lower than the one resulted from using the Random Forest algorithm. The sensitivity level and Specificity are 67% and 72% respectively. The two measures have also decreased in contrast with the previous model, but the percentage for Specificity is still above 70% and given that the company is most interested in predicting well the **BANDS* instances, this model can still be considered satisfying. 


## Logistic Regression


The third model that has been considered is the Logistic regression. Logistic regression is binomial regression model widely used in machine learning when the dependent variable is categorical. As in the case of the other models, the model has been build using caret.
...
```{r}
confusion_matrix_Logistic <- confusionMatrix(data = pred, reference = testData$band_type, positive = "NOBAND")
confusion_matrix_Logistic
```


In the case of Logistic regression, the an accuracy has a level of 74%. The Sensitivity level and Specificity are 76% and 69% respectively. It is interesting as this model is better at predicting the **NOBANDS** rather than **BANDS**, as opposed to the two previous models. 
...
predict_SVM1 = predict(model_svm, newdata = testData)
...
confusion_matrix_SVM1 <- confusionMatrix(predict_SVM1, reference = testData$band_type,positive = "NOBAND")
...
This model provides the lowest results so far, in terms both of accuracy and specificity. As the logistic regression, this model does a better job at predicting the class **NOBANDS** than **BANDS**.

An interesting feature of this model is that as for the Random Forest, it provides a list of the variables that are considered to be the most important.

```{r}
varimp_svm <- varImp(model_svm)
plot_var_SVM1 <- plot(varimp_svm, main = "Variable Importance with SVM1")
plot_var_SVM1
```
...
predict_SVM2 <- predict(model_svm2, newdata = testData)


```

```{r}
confusion_matrix_SVM2= confusionMatrix( predict_SVM2, reference = testData$band_type, positive = "NOBAND")

confusion_matrix_SVM2


```
The second SVM model offers good percentages in terms of Accuracy, Sensitivity and Specificity. 


```{r}

varimp_svm2 <- varImp(model_svm2)
plot_var_SVM2 <- plot(varimp_svm2, main = "Variable Importance with SVM2")
plot_var_SVM2
```
...
```{r neural network model, message=FALSE, warning=FALSE, include=FALSE}
...
confusion_matrix_NN <-   confusionMatrix(results1, testData$band_type, positive = 'NOBAND')
confusion_matrix_NN
```
The Neural Network has less good results compared to the other models. This is not surprising, as normally this type of models works better when the dataset has big amount of data to learn on, and in our case the data set has a total of 363 instances (75% of the total of 485 data points).

```{r}

varimp_nnet <- varImp(model_nnet)
plot_var_Nn <- plot(varimp_nnet, main = "Variable Importance with Nnet")
plot_var_Nn



```
This model also provides a visualization of the features that are considered the most important features, but as the model fails to provide good results in terms of predictions, it is considered that the information would not be very helpful.
...
confusion_matrix_CART <- confusionMatrix(cart.pred, testData$band_type, positive = 'NOBAND')
confusion_matrix_CART
```


This classification tree is not the best model in term of accuracy, but it is quite easy to understand and to show to the clients. On the right hand side there are some really good predictions of class **NOBAND** with a lot of observations in this category. Almost 57% of the observations are in this situation with an accurate probability of at least 88%. So here is the classic example of the processus when everything works well. The tree allows also to spot the classic example of when the processus is defectuous, as there is one branch of the tree which predict the class **BAND** with an accurate score of 100%. This could be useful for the clients as an example of what type of value to avoid in the processus. The importance of the cylinder_no variable makes a lot of sense here, as it represent the first split of the tree. 

## Comparing Variable Importance plots

```{r, fig.height=c(15), fig.width=c(10)}
grid.arrange(plot_var_RF, plot_var_Nn,plot_var_SVM1, plot_var_SVM2,  nrow=2, ncol=2)

```

## Comparing the results of confusion matrix

```{r}
comparing_results <- rbind(round(confusion_matrix_RF$byClass,2), round(confusion_matrix_KNN$byClass,2), round(confusion_matrix_Logistic$byClass,2), round(confusion_matrix_SVM1$byClass,2), round(confusion_matrix_SVM2$byClass,2),round(confusion_matrix_NN$byClass,2), round(confusion_matrix_CART$byClass,2))

Method <- c("Random Forest", "Nearest Neighbor", "Logistic regression", "Support Vector Machine 1", "Support Vector Machine 1", "Neuronal Network", "Classification Tree")

comparing_results <- cbind(Method,comparing_results)%>%
kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed","bordered"),full_width = F, position = "left")%>%
  footnote(symbol = strwrap("Results confusion matrix"))
...
As for now the random forest has offered the best results in terms of accuracy, sensitivity and specificity. As the model uses over 20 variables, it is considered that it has a high level of complexity. For this reason, a new model is proposed, one that uses the first 10 variables that resulted to be the most important at predicting the variable of interest suing the Random Forest algorithm. 

According to the model, the variables 
- 'blade_pressure'
- 'press'
- 'press_speed'
- 'ink_pct'
...
are the top ten  attributes that explain the variable of interest. 

A new data base has been created in which information has been retained for the above attributes, and has been stored in the variable 'bands.rf'.
...
Next, a new model has been build using the newly created data set. The model uses the Random Forest algorithm. The steps related to the data partition into a train and test set have been applied here as well. For the train control, the previous option have been 
...
model_rf.imp <- caret::train(band_type ~ .,
                         ...
                         trControl=trainControl(method="repeatedcv", number=10,
                                                repeats=10, verboseIter=FALSE, sampling = "down"))
prediction_simpler_model <- predict(model_rf.imp, testData.rf)
...
```{r}
confusion_matrix_RF_simple=confusionMatrix(prediction_simpler_model, testData.rf$band_type, positive = 'NOBAND')
```


```{r}

comparing_results <- rbind(round(confusion_matrix_RF$byClass,2), round(confusion_matrix_RF_simple$byClass,2))

Method <- c("Random Forest", "Random Forest simplified")
...
The results are encouraging. The balance accuracy has decreased slightly , and the specificity actually increased as opposed to the complex model. In turn, the sensitivity dropped down to 0.67% from 0.77%. As the company is interested to predict as best as possible the **BANDS**. Having a model that is less complex and with a high specificity, this last model is recommended to the company.

# Limitations and Recommendation

## Standardization of the records

The original dataset had a lot of shortcomings. First of all, only 51% of the rows have been complete. This translates in precious information that has been lost. Missing values have been replaced using an imputation method or have been deleted. In order to resolve this shortcoming, it is recommended to build a standardized internal reporting system that is shared in all the locations, in which the users fill in the data using predefined variable attributes.Also, the system should not allow the user the close the record without having recorded all the required information. 

Having data is very important for any company in order to derive insights. But is as important to have clean and reliable data, as it allows to gain time in the analysis process and also to rely on more information in the model building. 

## Improve CART model if possible

The above analysis showed that models as Random Forest and Support Vector Machine are offering good results in terms of predictions. Nevertheless, these methods are hard to understand by those that do not have a background in the filed of analytics. It is important that the solutions that a data scientist is creating, to answer the business problem. In the case of the printing company, employees that are working directly with the machines should be able to use and interpret  the models that have been delivered. A classification tree would be very useful, as it is easy to interpret and detect if a product will have a **BAND** or **NOBAND**.


## Investigation on the variable cylinder_no

A further investigation needs to be made for the variable cylinder_no, the assumption that the first letter represents the location of the production or the number of the batch make sense but is not certified. An discussion with the clients could be useful to understand better the situation. Some features of the variable have some real problem, as some value are always producing defectuous product, so it would mean that all the batch is defectuous. If it is the case, a lot of problem could be avoid.

## Simplification of the models
A recommendations has to be made about the simplification of the models. In this analysis, it has been tried to simplify the model keeping in consideration 11 variables. This number is maybe still too high for the clients, as they maybe want a more simpler model with 4 or 5 precise variables. This could have been possible with a discussion with them.


## Conclusion
During the project, the team has cleaned, analysed and build models in order to detect predict the product that will have a defect at the end of the printing run. Different models have been tested and some of them as Random Forest and Support Vector Machine showed promising results in terms of predicting the variable of interest. A simplified model was tested using the classification of the importance variables that resulted from the Random Forest model and the results have been encouraging. A model with 10 variable has been proposed for the company with a good specificity rate.

Whenever a model is built to help automatize a certain task, there are instances when the algorithm fails to predict the true values of the outcome. In some situations, there is no harm done by having false positives  or false negatives, but more than often there is an impact (for example: that predicting BAND and not having a BAND can also cost the company in the sense that the machine will be stopped and investigated, thus idle time on the machines and the time of the employee). The effect can be of monetary nature or is of non-quantitative shape. Nevertheless, the cost of classified outcomes can play a big role in the survival of a business or its reputation. This is why it is important for whomever uses an automated process to implement a feedback loop. That being said, if the company will decide to go through with the proposed model, the validity of it should be tested  with new data. It is even recommended that two or three models to be run in parallel and see their efficiency. A model that ran very well on the test set might very well not perform well on new data. Also a model that gave poor results, could actually predict very well the new instances. Both this scenario will bring a lot of insight to the analyst, who should use it in order to improve the model and understand better the interaction between the variables.
