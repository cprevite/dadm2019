...
trainDataMars <- trainData[, -1]
testDataMars <- testData[, -1]
...
trainDataRF <- trainData[, -1]
testDataRF <- testData[, -1]

integ_mod_1 <- data_integrity_model(data = bands[,-1], model_name = "randomForest")
integ_mod_1$data_ok
...
rf.pred<-predict(model_rf, testDataRF, type="class")
rf.pred
...
# delete cylinder_no because too much variation
trainDataNnet <- trainData[, -c(1)]
testDataNnet <- testData[, -c(1)]


model_nnet <- caret::train(band_type ~ ., data = trainDataNnet, method = "nnet", preProcess = "range", 
...
results1 <- predict(model_nnet, newdata = testDataNnet)
results1
